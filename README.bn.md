...
## ⚡ Claude Code দিয়ে ইনস্টল করুন (vibe-code)

<!-- vibe-install -->
কোনো টার্মিনালের দরকার নেই — পুরো কাজটা **Claude Code**-কেই করতে দিন। নিচের এই একটি প্রম্পট
একটি Claude Code সেশনে পেস্ট করুন এবং প্রতিটি ধাপে `y` দিয়ে উত্তর দিন। এটি সঠিক বাইনারি নামিয়ে আনে,
আপনার রিপোর জন্য মানচিত্র বিল্ড করে, ইন্টিগ্রেশন সেট আপ করে, এবং গ্রাফটি খুলে দেয়:

```text
আমার জন্য amazopic-এর graffiti ইনস্টল করো। github.com/amazopic/graffiti-এর সর্বশেষ রিলিজ থেকে আমার OS/arch-এর জন্য সঠিক স্ট্যাটিক বাইনারিটি ডাউনলোড করো (অথবা Go থাকলে `make build` দিয়ে সোর্স থেকে বিল্ড করো), এটিকে `graffiti` নামে আমার PATH-এ রাখো, এবং `graffiti version` দিয়ে যাচাই করো। তারপর মানচিত্র বিল্ড করার জন্য আমার রিপোর রুটে `graffiti .` চালাও, graffiti-কে Claude Code-এর সাথে যুক্ত করতে `graffiti init --hook` চালাও, এবং সবশেষে `.graffiti/map.html` খোলো যাতে আমি গ্রাফটি দেখতে পারি। প্রতিটি ধাপের আগে জিজ্ঞাসা করো।
```

<!-- quickstart -->
## কুইকস্টার্ট (৬০ সেকেন্ড)

```bash
# 1 — ইনস্টল করুন (অথবা `make build` দিয়ে সোর্স থেকে বিল্ড করুন)
curl -fsSL https://raw.githubusercontent.com/amazopic/graffiti/main/scripts/install.sh | sh

# 2 — আপনার রিপো মানচিত্রিত করুন (.graffiti/map.json, MAP.md, map.html লেখে)
cd your-repo
graffiti .

# 3 — গ্রাফটি দেখুন
open .graffiti/map.html        # macOS — Linux-এ `xdg-open`, Windows-এ `start` ব্যবহার করুন

# 4 — এটিকে প্রশ্ন জিজ্ঞাসা করুন: কোনো LLM নেই, কোনো API কী নেই
graffiti query "where is the user authenticated"
```

তারপর একবার এটিকে আপনার AI সহকারীর সাথে যুক্ত করুন:

```bash
graffiti init --hook    # Claude Code: skill + CLAUDE.md + grep→query নাজ
graffiti serve          # অথবা stdio-এর মাধ্যমে যেকোনো MCP ক্লায়েন্টের কাছে মানচিত্রটি প্রকাশ করুন
```

**আরও উদাহরণ প্রশ্ন** — `query` একটি সফট ~2,000-টোকেন বাজেটের মধ্যে একটি স্কোপড সাবগ্রাফ
ফেরত দেয়, ফলে কনটেক্সট ছোট ও সস্তা থাকে (প্রশ্নটি উদ্ধৃতিতে রাখুন):

```bash
graffiti query "login handler"
graffiti query "what does the checkout flow touch"
graffiti query "where is the cart fetched" ../shop   # অন্য একটি পাথ লক্ষ্য করুন
```
<!-- /quickstart -->
...
<!-- system-walkthrough -->
### ধাপে ধাপে সার্ভিসের একটি ফোল্ডার

ধরা যাক আপনার সার্ভিসগুলো একটি প্যারেন্ট ফোল্ডারে থাকে, প্রতিটি নিজের ডিরেক্টরিতে:

```text
myproject/                ← প্যারেন্ট ফোল্ডার = শেয়ার্ড "সিস্টেম স্টোর"
├── orders/               ← একটি সার্ভিস (Go)
├── web/                  ← একটি সার্ভিস (React/TS)
└── payments/             ← একটি সার্ভিস (Python)
```

**1. প্রতিটি সার্ভিস বিল্ড ও পাবলিশ করুন** প্যারেন্ট ফোল্ডারের একটি স্টোরে (`--to .`)।
`publish` একটি বিদ্যমান মানচিত্র পুনঃব্যবহার করে, তাই কোড পরিবর্তন ধরতে আগে বিল্ড করুন:
...
সার্ভিসের নাম ডিফল্টভাবে এর ফোল্ডারের নাম হয়; `--as <name>` দিয়ে ওভাররাইড করুন।

> ⚠️ **পুনঃপাবলিশের সময়:** `publish` একটি বিদ্যমান মানচিত্র **পুনরায় বিল্ড করে না**। কোড
> পরিবর্তনের পর, সবসময় আগে `graffiti build <service>` চালান (উপরের লুপটি তা-ই করে)
> এবং তারপর `publish` — অন্যথায় আপনি একটি পুরোনো মানচিত্র পাবলিশ করবেন।

**2. সিস্টেম গ্রাফ বিল্ড করুন** — মানচিত্রগুলো ফেডারেট করুন এবং লিঙ্কগুলো স্বয়ংক্রিয়ভাবে আবিষ্কার করুন:
...
**প্যারেন্ট ফোল্ডারে যা তৈরি হয়:**
...
**লিঙ্কের নির্ভুলতা উন্নত করুন।** স্বয়ংক্রিয় শনাক্তকরণ Go (net/http, gin/chi/echo),
Flask, FastAPI, Django/DRF, Spring, NestJS, ASP.NET, Ktor, ফ্রন্টএন্ড ক্লায়েন্ট
(React/Vue/Angular/Svelte), gRPC এবং Kafka/NATS কভার করে। যেখানে এটি যথেষ্ট নয়, সেখানে
একটি সার্ভিস রুটে এগুলোর একটি রাখুন (সর্বোচ্চ আত্মবিশ্বাস আগে):
...
ন্যূনতম `graffiti.contract.json`:
...
**ডেড এন্ডপয়েন্টে CI গেট করুন** — `audit` নন-জিরো এক্সিট করে যখন একটি কনজিউমার এমন একটি
এন্ডপয়েন্টে নির্দেশ করে যা কিছুই সরবরাহ করে না:

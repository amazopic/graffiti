...
## ⚡ Claude Code के साथ install करें (vibe-code)

<!-- vibe-install -->
किसी terminal की ज़रूरत नहीं — पूरा काम **Claude Code** को करने दें। नीचे दिया गया यह एक prompt
किसी Claude Code session में paste करें और हर step पर `y` का उत्तर दें। यह सही binary लाता है,
आपके repo के लिए map बनाता है, integration को जोड़ता है, और graph खोल देता है:

```text
मेरे लिए amazopic का graffiti install करें। github.com/amazopic/graffiti के latest release से मेरे OS/arch के लिए सही static binary download करें (या अगर Go उपलब्ध है तो `make build` से source से build करें), उसे `graffiti` के रूप में मेरे PATH पर रखें, और `graffiti version` से verify करें। फिर map बनाने के लिए मेरे repo root पर `graffiti .` चलाएँ, graffiti को Claude Code में जोड़ने के लिए `graffiti init --hook` चलाएँ, और अंत में `.graffiti/map.html` खोलें ताकि मैं graph देख सकूँ। हर step से पहले पूछें।
```
...
```bash
# 1 — install करें (या `make build` से source से build करें)
curl -fsSL https://raw.githubusercontent.com/amazopic/graffiti/main/scripts/install.sh | sh

# 2 — अपने repo का map बनाएँ (.graffiti/map.json, MAP.md, map.html लिखता है)
cd your-repo
graffiti .

# 3 — graph देखें
open .graffiti/map.html        # macOS — Linux पर `xdg-open`, Windows पर `start` इस्तेमाल करें

# 4 — इससे प्रश्न पूछें: कोई LLM नहीं, कोई API key नहीं
graffiti query "where is the user authenticated"
```

फिर इसे अपने AI assistant में एक बार जोड़ लें:

```bash
graffiti init --hook    # Claude Code: skill + CLAUDE.md + grep→query nudge
graffiti serve          # या किसी भी MCP client को stdio के ज़रिए map उपलब्ध कराएँ
```

**और उदाहरण प्रश्न** — `query` एक soft ~2,000-token budget के भीतर एक scoped subgraph लौटाता है,
इसलिए context छोटा और सस्ता बना रहता है (प्रश्न को quotes में रखें):

```bash
graffiti query "login handler"
graffiti query "what does the checkout flow touch"
graffiti query "where is the cart fetched" ../shop   # किसी अन्य path को target करें
```
<!-- /quickstart -->
...
## 🛰️ System orchestration — कई services, एक graph

<!-- system-orchestration -->
एक microservice system कई स्वतंत्र repos होती है जो मिलकर एक product बनाती हैं। graffiti
हर एक का नक्शा बनाती है, फिर हर service की *contract surface* (वह क्या `provides` यानी प्रदान करती है
और क्या `consumes` यानी उपभोग करती है) से **उनके बीच के edges को खोज निकालती है** — HTTP, gRPC, queues।
कोई हाथ से जोड़-तोड़ नहीं: हर service अपना नक्शा स्वयं publish करती है; orchestrator publish किए गए
artifacts को federate करता है और consumers को providers से match करता है।

```bash
# हर service की CI में (या locally) — उसका नक्शा एक साझा store में publish करें:
graffiti publish --to ../system-store --as carts

# फिर, CI में या माँग पर, पूरे system के ऊपर:
...
हर नक्शा एक **contract surface** साथ ले जाता है जो `openapi.json`, `.proto`,
framework routes, queue calls, या एक स्पष्ट `graffiti.contract.json` से निकाली जाती है। Cross-service
links को confidence के आधार पर score किया जाता है; **ambiguous** (अस्पष्ट) और **dangling** (dead-endpoint, यानी
मृत-endpoint वाले) consumers की रिपोर्ट दी जाती है, उन्हें कभी चुपचाप गिराया नहीं जाता। System store बस एक directory
या git repo है — $0, offline, पुनः-गणना योग्य।
[design doc](docs/superpowers/specs/2026-06-24-graffiti-system-orchestration-design.md) देखें।

<!-- system-walkthrough -->
### services का एक folder, चरण-दर-चरण

मान लीजिए आपकी services एक ही parent folder में रहती हैं, हर एक अपनी directory में:

```text
myproject/                ← parent folder = साझा "system store"
├── orders/               ← एक service (Go)
├── web/                  ← एक service (React/TS)
└── payments/             ← एक service (Python)
```

**1. हर service को build और publish करें** parent folder पर बने एक store में (`--to .`)।
`publish` किसी मौजूदा map का पुनः-उपयोग करता है, इसलिए code परिवर्तनों को शामिल करने के लिए पहले build करें:
...
service का नाम डिफ़ॉल्ट रूप से उसके folder का नाम होता है; `--as <name>` से इसे बदलें।

> ⚠️ **पुनः-publish पर:** `publish` किसी मौजूदा map को फिर से build **नहीं** करता। code
> बदलने के बाद, हमेशा पहले `graffiti build <service>` चलाएँ (ऊपर का loop ऐसा ही
> करता है) और फिर `publish` — वरना आप एक बासी map publish कर देंगे।

**2. system graph बनाएँ** — maps को federate करें और links को auto-discover करें:
...
graffiti system impact orders   # orders बदलने पर कौन टूटता है (direct + transitive)
graffiti system audit           # dangling consumers · orphan providers · ambiguous (non-zero exit → CI gate)
graffiti system status          # पिछले build के बाद से कौन-सी services drift हुईं
graffiti system query "where is the order created"   # पूरे system में LLM-रहित retrieval
graffiti system list            # registered services
```

**parent folder में क्या उतरता है:**

```text
myproject/.graffiti-system/
├── system.json                 # services की registry (इसे commit करें)
├── overlay.json                # discovered links (व्युत्पन्न — .gitignore करना सुरक्षित)
├── system.html                 # visual system map
└── services/<name>/map.json    # हर service का published map
```

**link accuracy सुधारें।** Auto-detection Go (net/http, gin/chi/echo),
Flask, FastAPI, Django/DRF, Spring, NestJS, ASP.NET, Ktor, frontend clients
(React/Vue/Angular/Svelte), gRPC और Kafka/NATS को कवर करता है। जहाँ यह पर्याप्त न हो, वहाँ
इनमें से एक को किसी service root में डालें (सबसे अधिक confidence वाला पहले):

| File | देता है |
|------|------|
| `graffiti.contract.json` | स्पष्ट `provides` / `consumes` — कोई भी stack, सबसे अधिक confidence |
| `openapi.json` / `swagger.json` | HTTP routes को `provides` के रूप में |
| `*.proto` | gRPC methods को `provides` के रूप में |

न्यूनतम `graffiti.contract.json`:
...
**dead endpoints पर CI को gate करें** — जब कोई consumer ऐसे endpoint की ओर इंगित करता है
जिसे कुछ भी provide नहीं करता, तब `audit` non-zero exit करता है:

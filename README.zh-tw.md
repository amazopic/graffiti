...
```bash
# 1 — 安裝（或用 `make build` 從原始碼建置）
curl -fsSL https://raw.githubusercontent.com/amazopic/graffiti/main/scripts/install.sh | sh

# 2 — 描繪你的儲存庫（寫出 .graffiti/map.json、MAP.md、map.html）
cd your-repo
graffiti .

# 3 — 觀看圖譜
open .graffiti/map.html        # macOS — Linux 用 `xdg-open`，Windows 用 `start`

# 4 — 向它提問：無 LLM、無 API 金鑰
...
graffiti serve          # 或透過 stdio 將地圖開放給任何 MCP 用戶端
```

**更多範例問題** — `query` 會在約 2,000 token 的軟性預算內回傳一個範圍受限的
子圖，因此脈絡能維持精簡又省成本（問題請加上引號）：

```bash
graffiti query "login handler"
graffiti query "what does the checkout flow touch"
graffiti query "where is the cart fetched" ../shop   # 指向另一個路徑
```
<!-- /quickstart -->
...
假設你的各個服務都放在同一個父資料夾裡，各自有自己的目錄：

```text
myproject/                ← 父資料夾＝共享的「系統儲存區」
├── orders/               ← 一個服務（Go）
├── web/                  ← 一個服務（React/TS）
└── payments/             ← 一個服務（Python）
```

**1. 建置並發布每個服務**到父資料夾的一個儲存區（`--to .`）。
`publish` 會重用既有的地圖，所以請先 build 以納入程式碼變更：
...
服務名稱預設為其資料夾名稱；可用 `--as <name>` 覆寫。

> ⚠️ **關於重新發布：**`publish` **不會**重新建置既有的地圖。在
> 變更程式碼後，請務必先執行 `graffiti build <service>`（上面的迴圈
> 就是這麼做的），再執行 `publish` — 否則你發布的會是過時的地圖。

**2. 建置系統圖譜** — 將各個地圖聯邦起來，並自動探查連結：
...
**提升連結準確度。**自動偵測涵蓋 Go（net/http、gin/chi/echo）、
Flask、FastAPI、Django/DRF、Spring、NestJS、ASP.NET、Ktor、前端用戶端
（React/Vue/Angular/Svelte）、gRPC 與 Kafka/NATS。當這還不夠時，把
下列其中之一放進服務根目錄（信心由高到低）：
...
**為失效端點把關 CI** — 當某個消費端指向沒有任何服務提供的端點時，
`audit` 會以非零碼結束：

...
- **MCP (amended 2026-06-16):** a **hand-rolled minimal JSON-RPC 2.0 server over newline-delimited stdio, stdlib only** (`encoding/json` + `bufio` + `io`), isolated behind `internal/mcp.Serve(r io.Reader, w io.Writer)`. The original "Go MCP SDK" intent is **superseded**: the official SDK pulls ~10 transitive deps (JWT/OAuth2/HTTP-auth) that a stdio-only, keyless, offline server needs none of, contradicting the single-static-pure-Go / zero-runtime-deps ethos and the "no auth, no HTTP MCP server" non-goal (§4). graffiti speaks exactly `initialize` + `tools/list` + `tools/call`; the SDK remains a documented drop-in fallback behind the `Serve` seam. (Same isolate-and-pivot pattern as the §10 parser amendment.)
...
The **federation engine + signal 1 (explicit links)** shipped, in a **purely additive** `internal/workspace` package (zero changes to the single-project build/schema/extraction). `graffiti link <a> <b>` builds any unbuilt member, auto-discovers the workspace root (nearest common ancestor; `FindRoot` walks up from cwd on later runs), writes a committable `workspace.json` registry, and computes `overlay.json`. `graffiti query/serve/update --workspace`, `workspace add/rm/list`, `links check`, and `federate --explain` are wired. **Federated query reuses `query.Query` verbatim** over an in-memory **alias-prefixed combined `store.Index`** (members loaded with `alias::` prefixes + the overlay's cross-edges) — the alias prefix never touches any project's `map.json`. Unresolved explicit links are recorded, never emitted as confident edges (honesty-first); a staleness nudge fires when a member changed since the overlay was computed. Determinism per §16.5 (members sorted by alias, links by `(from,to,relation)`, byte-identical modulo `generated_at`).

**Decisions / deviations (zero-dep ethos — isolate-and-pivot like §10):** the explicit-links file is a **line-based format** `<root>/.graffiti-workspace/links` (`alias::id -> alias::id [relation]`, `#` comments), **not YAML** (no YAML dependency); §16.2's `links.yml` is superseded for v1. Members are resolved by **alias** (the §16.6 immutable project id is deferred). `workspace.json` (committable) and `overlay.json` (gitignored) both live under `<root>/.graffiti-workspace/`. Federated query uses a **single budget** (the §16.3 two-tier budget is deferred). **Deferred to follow-on plans:** auto-detection signals 2–5 (each needs a per-project contract-surface carrier added to `map.json`), the §16.6 immutable id, the two-tier budget, `graffiti init` workspace-awareness, and the §16.7 lanes viewer.
```
